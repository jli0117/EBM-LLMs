...
### RCT summarizaiton 
Generating high-quality narrative summaries that synthesize biomedical evidence from studies is crucial for enabling EBM. These summaries allow practitioners to quickly understand the consolidated findings, strengths, limitations, and implications across all relevant clinical trials on a given topic.

The input to LLMs consists of the titles and abstracts of the RCTs included in a given Cochrane systematic review. The gold standard target is the \textit{Authors' Conclusions} section from the abstract of that Cochrane review.
