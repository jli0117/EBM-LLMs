...
### Biomedical QA
Biomedical question answering is the task of answering natural language questions about biomedical topics using relevant information from a given corpus of text. The dataset for biomedical QA can be downloaded from [link].: https://microsoft.github.io/BLURB/tasks.html#dataset_bioasq-taskb

### RCT summarizaiton 
Generating high-quality narrative summaries that synthesize biomedical evidence from studies is crucial for enabling EBM. These summaries allow practitioners to quickly understand the consolidated findings, strengths, limitations, and implications across all relevant clinical trials on a given topic. The input to LLMs consists of the titles and abstracts of the RCTs included in a given Cochrane systematic review. The gold standard target is the Authors' Conclusions section from the abstract of that Cochrane review. The dataset for PICO extraction can be downloaded from: https://github.com/bwallace/RCT-summarization-data

### Medical text simplification
Medical text simplification refers to the process of simplifying complex medical text to make it more understandable, especially for patients and caregivers with limited health literacy. The dataset for PICO extraction can be downloaded from: https://github.com/nikhilpriyatam/medical_text_simplification/tree/master/data
