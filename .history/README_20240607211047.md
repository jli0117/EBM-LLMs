...
You can modify the parameters in the corresponding Jupyter notebook to get results. Specifically, `GPT` allows you to select different large language models to use for the tasks.  `prompt_type`  parameter controls the prompting style used for the tasks. The available options include:

andfor changing the prompgintg styles, including `base`, `1-shot`, `3-shot`, `5-shot`, and `guided`.
